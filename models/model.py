...
#     model = Qwen2.from_pretrained(model_name).to("cuda")
...
if __name__ == "__main__":
    def print_filtered_models():
        from huggingface_hub import list_models

        forbidden_endings = ["Int4", "Int8", "AWQ", "GGUF", "MLX"]
        forbidden_keywords = ["Audio", "72B"]  # audio aren't support yet. also 72B is too big so i'll test it manually later
        models = list_models(author="Qwen")
        model_list = []

        for model in models:
            model_id = model.modelId
            # Only print if it starts with "Qwen/Qwen2" and does NOT end with any forbidden suffix
            if (
                model_id.startswith("Qwen/Qwen2")
                and not any(model_id.endswith(end) for end in forbidden_endings)
                and not any(keyword in model_id for keyword in forbidden_keywords)
            ):
    ...
    # print(print_filtered_models())
    for model in print_filtered_models():
        print(model)

    print(len(print_filtered_models())) # 46
